...
#     # df_team: week_num 없어짐 -> groupby(KPI)로 집계
#     df_team = (
...
#             # --- (B1) HWK Total인 경우: df_team에는 Week_num이 없음 -> KPI만 필터
#             if selected_team_detail == "HWK Total":
...
#             # --- (B5) Line3: 이전 주 대비 증감
#             if (val_last is not None) and (not np.isnan(val_last)) and (val_prev is not None) and (not np.isnan(val_prev)):
#                 delta_actual = val_last - val_prev
#                 emoticon = get_trend_emoticon(kpi, delta_actual)
#                 if pd.notna(final_last) and not np.isnan(final_last):
#                     # 이전 주 final 점수(개별 팀인 경우만)
#                     if selected_team_detail != "HWK Total":
#                         df_prev_fin = df_cum[(df_cum["Week_num"] == (latest_week - 1)) & (df_cum["KPI"] == kpi)]
#                         prev_final = df_prev_fin["Final"].mean() if not df_prev_fin.empty else np.nan
...
#                     else:
#                         # HWK Total은 final_last가 있을 수 있지만 누적 점수와 달라질 수 있으니
#                         line3 = f"{emoticon}{format_value_with_unit(delta_actual, kpi_unit)}"
#                 else:
#                     line3 = f"{emoticon}{format_value_with_unit(delta_actual, kpi_unit)}"
#             else:
